...
# 问题转换
- 正好K个的方案数 = 至少K个的方案数 - 至少K-1个的方案数 = 至多K个的方案数 - 至多K-1个的方案数

# 背包 knapsack
- 记忆化搜索建议反向迭代，target建议扣除
    - 反向迭代正好对应动态规划正向迭代
    - 使用扣除可以合并至多，正好两种情况；使其都是关于0的特判
    - 动态规划建议枚举`dp[i]`而不是`dp[i-1]`这样和记忆化搜索条件一模一样
...
```python
#01背包改为
#target为0实际上代表的不是正好是0
#而是已经满足之少amount条件
#错误写法: target 取值可能有很多负值 但是所有负值都只代表方案已经满足，记忆化搜索缓存会很大
@cache
def dp(i, target):
    if i < 0:
        return 0 if target <= 0 else 1<<31
...
#正确写法: 通过取max将所有负值都合并到0
@cache
def dp(i, target):
    if i < 0:
        return 0 if target == 0 else 1<<31
    else:
        return min(dp(i - 1, max(target - coins[i], 0)) + 1, dp(i - 1, target))
...
def dp(i, target):
    if i < 0:
        ...
        return min(dp(i, min(target - coins[i], 0)) + 1, dp(i - 1, target))
